...
const MoviesPage = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        setLoading(true);
        const res = await authenticatedFetch(`${ANIMEPLAY_API_BASE_URL}/movies`);
        const json = await res.json();
        
        if (json.status === 'success' && Array.isArray(json.data)) {
          setMoviesList(mapApiData(json.data));
        }
      } catch (error) {
  ...
  const currentList = moviesList.slice(startIndex, startIndex + ITEMS_PER_PAGE);

  if (loading) return <Loader message="SCOUTING CINEMATIC ARCHIVES..." />;

  return (
    <div className="max-w-7xl mx-auto p-4 space-y-12">
      <header className="bg-[#FFCC00] border-8 border-black p-8 md:p-12 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] transform -rotate-1 relative overflow-hidden text-black">
        <div className="absolute top-0 right-0 opacity-10 font-black text-8xl -translate-y-4 uppercase">Movies</div>
        <h1 className="text-4xl md:text-7xl font-black oswald italic relative z-10">FILM LAYAR LEBAR</h1>
        <p className="text-lg md:text-2xl font-bold oswald text-white bg-black px-2 py-1 inline-block mt-4 relative z-10 uppercase">THE BIG SCREEN EXPERIENCE</p>
      </header>
